...
**Color:** `client/src/index.css` `@theme`. A teal-to-emerald ramp (`brand-50`
...
**Spacing:** Tailwind's default scale, used consistently: `p-4` to `p-6` inside
...
`border-gray-800 bg-gray-900` (dark), hover state adds `shadow-xl` plus a
...
**Search** leads to popular-city pills, then featured listings (skeleton,
then real cards, then an empty state if needed; never a blank gap or a spinner).
...
their image, and deepen their shadow, since a card is the single most-repeated
...
Two-column layout: images, info, and description on the left (2/3 width),
sticky-feeling inquiry form on the right (1/3 width), since the inquiry form
is the conversion point, so it stays visible without scrolling past it.
...
`ScoreRing` (not just a toast), since the score is data the customer's own
...
gradient accent, icon, and animated count, three charts (monthly inquiries
...
list. Leads tab shows the full score breakdown (`ScoreRing` plus four animated
...
- **Dead links / features implied but not built.** The nav here does not
  ...
  see the IA table above. Every nav item resolves to a real page with real
  data. Author's note: a project that looks 90% done because every icon and
  label exists, but a third of them go nowhere, reads worse under scrutiny
  than one that's honest about a smaller, fully-working surface.
...
- **No dark mode, or dark mode as an afterthought that breaks contrast.**
  Implemented via a single `dark` class plus Tailwind's `@custom-variant`, and
  ...
  computation, not "invert and hope." Enable it now for a genuinely nice
  touch: recruiters do look for it, and it's already load-bearing here, not
  bolted on.
